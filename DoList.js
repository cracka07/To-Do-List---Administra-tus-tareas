...
// fecha.addEventListener("click",Date);


 // var f_fecha=e.target;
 let date = new Date();
let result = date.toLocaleTimeString();
let result1 = "Local date: " + date.toLocaleDateString()

let fecha=document.getElementById("date")
 fecha.innerHTML = result1;
 fecha.innerHTML += ", " + result;
...
// Tu código acá:
  let agregar=document.querySelector("#createdBy");
agregar.innerHTML += "Mariano" 
...
function buildToDo(todo, index) {
   ...
   // toDoText.setAttribute("id", index) // le asigno un id index(parámetro) a toDoText
    ...
    toDoShell.appendChild(toDoText) //Agrega un nuevo nodo al final de la lista de un elemento hijo de un elemento padre especificado.
...
var guardar=buildToDos(toDoItems);

//ftoDoContainer.appendChild(el)
guardar.forEach(el=>{
...
var valor=toDoInput.value;
toDoInput.value=""; 

var newToDo=new ToDo(valor);
if (valor){
...
function completeToDo(event) {
  ...
  const index = event.target.id; // lo uso mucho en react
  // Tu código acá:
